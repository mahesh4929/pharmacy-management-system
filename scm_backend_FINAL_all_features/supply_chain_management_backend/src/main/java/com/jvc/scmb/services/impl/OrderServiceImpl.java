...
public class OrderServiceImpl implements OrderService {
	...
	public OrderResponseDto addOrder(OrderRequestDto orderRequestDto, String token) {
		log.info("New order request received: customerId={}, itemCount={}", 
			orderRequestDto.getCustomer_id(), 
			orderRequestDto.getOrdered_items() != null ? orderRequestDto.getOrdered_items().size() : 0);
		//verify jwt from header of request
		token = JwtVerification(token);
